...
if (isset($_POST['update'])) {
    ...
    if (!empty($title) && !empty($content)) {
        ...
        if (empty($imageName)) {
            ...
            $sql = "SELECT image1 FROM post WHERE postid = ?";
            ...
            if ($result->num_rows > 0) {
                ...
                $imageName = $post['image1']; // Keep old image
            }
            $stmt->close();
        }

        // Update post in the database
        $sql = "UPDATE post SET title = ?, postcate = ?, content = ?, image1 = ?, created_at = ? WHERE postid = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('sssssi', $title, $postcat, $content, $imageName, $created_at, $postid);

        if ($stmt->execute()) {
            $msg = "<div class='text-success'>Post updated successfully.</div>";

            // Delete old image when a new one is uploaded
            if (!empty($imageName) && !empty($post['image']) && file_exists('../uploads/' . $post['image1'])) {
                unlink('../uploads/' . $post['image1']); // Delete the old image
            }
        ...
        <div class="row align-center">
          <div class="col-xl-6 col-lg-8 offset-xl-3 pl-30 pl-md-15 pl-xs-15">
            <ul class="item-flex">
              ...
              <li>
                <a href="tel:+2348149646850"
                  ><i class="fas fa-phone-alt"></i> +2348149646850</a
                >
              </li>
            </ul>
          </div>
              ...
              <h5 class="sub-title">UPDATE POST</h5>
                ...
                <?php
                    ...
                    if (isset($_GET['postid'])) {
                        ...
                        $sql = "SELECT title, postcate, content, image1, created_at FROM post WHERE postid = ?";
                        $stmt = $conn->prepare($sql);
                        $stmt->bind_param('i', $postid);
                        $stmt->execute();
                        $result = $stmt->get_result();
                ...
                <div class="contact-form-style-one">
                    ...
                    <form action="" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <div class="text-primary pull-left">Title:</div>
                            <input type="text" name="title" id="title" class="form-control" value="<?php echo htmlspecialchars($post['title']); ?>" required>
                        </div>
                        <div class="form-group">
                            <div class="text-primary pull-left">Created At:</div>
                            <input type="text" name="created_at" id="created_at" class="form-control" value="<?php echo $post['created_at']; ?>" required>
                        </div>
                        <div class="form-group">
                            <div class="text-primary pull-left">Category:</div>
                            <input type="text" name="postcat" id="title" class="form-control" value="<?php echo htmlspecialchars($post['postcate']); ?>" required>
                        </div>

                        <div class="form-group">
                            <div for="content" class="text-primary p-1 pull-left">Content:</div>
                        ...
                        <div class="form-group">
                            ...
                            <?php if (!empty($post['image1'])): ?>
                                <div>
                                    <img src="../uploads/<?php echo $post['image1']; ?>" alt="Post Image" width="100">
                                    <div class="text-secondary">Current Image: <?php echo htmlspecialchars($post['image1']); ?></div>
                                </div>
                            <?php endif; ?>
